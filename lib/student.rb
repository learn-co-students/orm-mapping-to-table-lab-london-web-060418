

class Student

  # Remember, you can access your database connection anywhere in this class
  #  with DB[:conn]

  attr_accessor :name, :grade
  attr_reader :id

  def initialize(name, grade, id=nil)  #optional id here
    @id = id
    @name = name
    @grade = grade
  end

  #creates students table in the database:
  def self.create_table
    sql = <<-SQL
      CREATE TABLE IF NOT EXISTS students (
        id INTEGER PRIMARY KEY,
        name TEXT,
        grade INTEGER
      )
    SQL
    DB[:conn].execute(sql)
  end

  #drops the studens table from the database:
  def self.drop_table
    sql = <<-SQL
      DROP TABLE students
    SQL
    DB[:conn].execute(sql)
  end

  #bound paremeters passes student's name, grade into the SQL statement:
  #id column's value will be automatically assigned
  def save
    sql = <<-SQL
    INSERT INTO students (name, grade)
    VALUES (?, ?)
    SQL
    DB[:conn].execute(sql, name, grade)

    @id = DB[:conn].execute("SELECT last_insert_rowid() FROM students")[0][0]
    #grab the ID of the last inserted row,
    #the row you just inserted into the database,
    #and assign it to the be the value of the @id attribute of the given instance.
  end

  #takes in hash of attributes and creates a new student object 
  def self.create(name:, grade:)
    student = Student.new(name, grade)
    student.save     #saves student to the database
    student   #returns it
  end


end
